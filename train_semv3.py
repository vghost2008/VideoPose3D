...
from common.transform_model import *
...
if not args.disable_optimizations and not args.dense and args.stride == 1:
    ...
    model_pos_train = TransTemporalModelOptimized1f(poses_valid_2d[0].shape[-2], 
                                in_features,
                                dataset.skeleton().num_joints(),
                                filter_widths=filter_widths, causal=args.causal, dropout=args.dropout, channels=args.channels)
else:
    # When incompatible settings are detected (stride > 1, dense filters, or disabled optimization) fall back to normal model
    model_pos_train = TransTemporalModel(poses_valid_2d[0].shape[-2],
                                in_features,
                                dataset.skeleton().num_joints(),
                                filter_widths=filter_widths, causal=args.causal, dropout=args.dropout, channels=args.channels,
                                dense=args.dense)
#use 2d keypoints as inputs predict the relative of 3d coordinate of all keypoints in camera coordinate    
model_pos = TransTemporalModel(poses_valid_2d[0].shape[-2],
                            in_features,
                            dataset.skeleton().num_joints(),
                            filter_widths=filter_widths, causal=args.causal, dropout=args.dropout, channels=args.channels,
                            dense=args.dense)
...
if semi_supervised:
    from common.sem_dataset import SemDataset
    dataset = SemDataset()
    cameras_semi, poses_semi_2d = dataset.cameras_data,dataset.poses_2d
    
    if not args.disable_optimizations and not args.dense and args.stride == 1:
        # Use optimized model for single-frame predictions
        model_traj_train = TransTemporalModelOptimized1f(poses_valid_2d[0].shape[-2], 
                in_features,
                1,
                filter_widths=filter_widths, causal=args.causal, dropout=args.dropout, channels=args.channels)
    else:
        # When incompatible settings are detected (stride > 1, dense filters, or disabled optimization) fall back to normal model
        model_traj_train = TransTemporalModel(poses_valid_2d[0].shape[-2],
                in_features, 1,
                filter_widths=filter_widths, causal=args.causal, dropout=args.dropout, channels=args.channels,
                dense=args.dense)

    #use 2d keypoints as inputs predict the first keypoint's trajectory in camera coordinate
    model_traj = TransTemporalModel(poses_valid_2d[0].shape[-2],
                        in_features,
                        1,
                        filter_widths=filter_widths, causal=args.causal, dropout=args.dropout, channels=args.channels,
                        dense=args.dense)
...
while epoch < args.epochs:
    ...
    if semi_supervised:
        ...
        for (_, batch_3d, batch_2d), (cam_semi, _, batch_2d_semi) in \
            ...
            if any_nan(loss_3d_pos):
                print(f"loss_3d_pos nan")
                loss_total = torch.zeros_like(loss_3d_pos)
            else:
            ...
            if not skip:
                ...
                if not args.no_proj:
                    if any_nan(loss_reconstruction):
                        print(f"loss_reconstruction nan")
                    else:
                        loss_total += loss_reconstruction
                        loss_total += loss_reconstruction_offset
                ...
                print(f"{epoch}/{cur_iter}: 3d loss={loss_3d_pos_np:5.2f}, loss traj={loss_traj_np:5.2f}, recon loss={loss_reconstruction_np:5.2f}, recon loss offset={loss_reconstruction_offset_np}, bone penalty={penalty_np:5.2f}.")
            else:
                N_semi += 1 # To avoid division by zero

            loss_total.backward()
            torch.nn.utils.clip_grad_norm_(model_pos_train.parameters(), 16)
            torch.nn.utils.clip_grad_norm_(model_traj_train.parameters(), 16)
            optimizer.step()
...
'''
train_semv3.py --subjects-train S1,S11,S9,S5,S6,S7,S8  -e 100 -lrd 0.98 -arc 3,3,3,3 --warmup 3 -b 512 -k detectron_pt_coco --dataset h36m 
'''
